/**
 * Lead Scoring Service for Research Engine
 * Mirrors the main app's LeadScoringService but runs in the standalone Express server.
 */
...
export class LeadScoringService {
  ...
  private scoreCompanySizeFit(
    ...
    if (!leadSize || !targetRange) return 5;
    ...
    if (lead.min <= target.max && lead.max >= target.min) return 20;
    return 5;
  }
