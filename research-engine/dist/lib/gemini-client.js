...
class GeminiClient {
    constructor(encryptedApiKey, modelName = "gemini-2.0-flash") {
     ...
     * Starts a Deep Research task using the Interactions API
     */
    async createDeepResearchTask(prompt) {
        return this.retryWithBackoff(async () => {
            ...
            if (!response.ok) {
                ...
                const err = new Error(error.error?.message || `API Error: ${response.statusText}`);
     ...
     */
    async getDeepResearchStatus(interactionId) {
        return this.retryWithBackoff(async () => {
            ...
            if (!response.ok) {
                const error = await response.json();
                const err = new Error(`Status Poll Error: ${response.statusText}`);
                err.status = response.status;
                throw err;
            }
            return (await response.json());
    ...
    extractJson(text) {
        try {
            ...
            console.error("JSON Extraction Error. Raw text snippet:", text.substring(0, 200));
     ...
     */
    async refinePrompt(originalPrompt, scope) {
        const prompt = `
    ...
    `;
        try {
            ...
            if (error.status === 429) {
                throw new Error("API quota exceeded. Please wait a minute or try a different Gemini API key. Free tier limits: 15 requests/minute.");
            }
     ...
     */
    async retryWithBackoff(fn, maxRetries = 3, baseDelayMs = 2000) {
        let lastError;
        for (let attempt = 0; attempt < maxRetries; attempt++) {
            try {
                ...
                console.log(`[GeminiClient] Rate limited. Retry ${attempt + 1}/${maxRetries} in ${delayMs}ms`);
     ...
     */
    async analyzeSources(sources, researchGoal) {
...
Begin your analysis now:`;
        try {
            ...
            throw new Error("Analyis mission failed: " +
