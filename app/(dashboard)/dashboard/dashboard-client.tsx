...
export function DashboardClient({ initialData }: DashboardClientProps) {
  ...
  const { data: projectsData, isLoading: projectsLoading } = trpc.project.getProjects.useQuery();
  const { data: productivityTrends } = trpc.analytics.getProductivityTrends.useQuery({
    range: timeRange === "3days" ? "7d" : timeRange === "week" ? "7d" : "30d",
  });
  const [todayDate] = useState(() => new Date());
  const { data: dailyChecklist } = trpc.habit.getDailyChecklist.useQuery({ date: todayDate });
  ...
  const productiveMins = parseDurationToMinutes(tracklog?.productive_hours ?? "");
  const focusedMins = parseDurationToMinutes(tracklog?.focused_hours ?? "");
  const unproductiveMins = parseDurationToMinutes(tracklog?.unproductive_time ?? "");
  const totalMins = productiveMins + focusedMins + unproductiveMins;
  const productivePct = totalMins > 0 ? Math.round((productiveMins / totalMins) * 100) : 0;
  const focusedPct = totalMins > 0 ? Math.round((focusedMins / totalMins) * 100) : 0;
  const unproductivePct = totalMins > 0 ? Math.round((unproductiveMins / totalMins) * 100) : 0;

  const tracklogHours = (productiveMins + focusedMins) > 0 ? (productiveMins + focusedMins) / 60 : 0;
  const trendHoursSum = (productivityTrends || []).reduce((acc, p) => acc + (p.hours || 0), 0);

  const projects = projectsData ?? [];
  const activeProjectsList = projects.filter((p) => p.status === "active" || p.status === "planning");
  const totalCompletedTasks = taskStats?.completed ?? stats?.completedTasks ?? 0;
  ...
  const clientRevenueZAR = revenueData?.clientRevenue ?? revenueData?.pipelineValue ?? revenueData?.monthlyRevenue ?? 0;
  ...
  const topPriorityGoal = useMemo(() => {
    ...
    if (uncompletedToday.length > 0) {
      return [...uncompletedToday].sort((a, b) => (a.streak || 0) - (b.streak || 0))[0];
    }
  ...
  const trendMax = Math.max(...(productivityTrends?.map((p) => p.hours) ?? [1]), 1);
    ...
    <div className="w-full space-y-6 pb-12">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="bg-secondary p-1 rounded-full flex items-center shadow-inner">
            {(["3days", "week", "month"] as const).map((r) => (
              ...
              >
                {r === "3days" ? "Last 3 days" : r === "week" ? "Last Week" : "Last Month"}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1.5 flex-1">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground font-mono">
                Zero friction • 1-Tap Execution
              </span>
            </div>

            <h2 className="text-2xl font-extrabold text-foreground tracking-tight flex items-center gap-2">
              {topPriorityGoal ? topPriorityGoal.title : "No active focus session scheduled"}
            </h2>
          ...
          <div className="flex flex-wrap items-center gap-3 shrink-0">
              ...
              <>
                <button
                  onClick={() => {
                    ...
                    toast.success(`Focus Sprint launched for "${topPriorityGoal.title}" ⚡`);
                  }}
                  className="px-5 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-black font-extrabold text-xs font-mono flex items-center gap-2 shadow-lg shadow-amber-500/20 transition-all hover:scale-[1.02]"
                >
                  <Play className="w-4 h-4 fill-current" />
                  Start Focus Sprint ({topPriorityGoal.targetMinutes || 45}m)
                </button>

                <button
                  onClick={() => {
                    toggleGoalCompletion(topPriorityGoal.id);
                    toast.success(`🎉 "${topPriorityGoal.title}" completed! Streak updated!`);
                  }}
            ...
            <div className="px-3 py-2 rounded-xl bg-secondary/80 text-muted-foreground font-mono text-[11px] flex items-center gap-1.5 border border-white/5">
              <span className="font-bold text-foreground">⌘K</span> Quick Commands
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                No guilt for missed days. Here is where you left off: {(stats as any)?.todaysTasks ?? 0} tasks & {activeProjectsList.length} projects in flight.
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {topPriorityGoal && (
              <button
                onClick={() => {
                  startGoalSession(topPriorityGoal);
                  toast.success(`Picked up momentum on "${topPriorityGoal.title}" ⚡`);
                }}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <h2 className="text-4xl font-extrabold text-foreground mt-1 tracking-tight">
                {taskStats?.created ? `${taskStats.completed}/${taskStats.created}` : totalCompletedTasks} Tasks
              </h2>
              <p className="text-xs text-muted-foreground mt-0.5">
                Completed vs Created ({timeRange === "month" ? "This Month" : "This Week"})
              </p>
            </div>
            ...
            <div className="h-28 flex items-end justify-between gap-1.5 pt-4 px-2">
              {productivityTrends && productivityTrends.length > 0 ? (
                productivityTrends.map((pt, i) => {
                  const heightPercent = Math.max((pt.hours / trendMax) * 100, 8);
                  return (
                    <div
                      key={i}
                    ...
                    >
                      ...
                      <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 bg-black text-white text-[10px] p-2 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-20 shadow-xl border border-secondary font-mono">
                        ...
                        <p className="text-emerald-400">{pt.hours} hours logged</p>
                      </div>
                    </div>
                  );
                })
        ...
        <div className="toota-card space-y-4 flex flex-col justify-between">
          <div className="flex items-center justify-between">
            ...
            <Link href="/projects" className="text-xs text-emerald-500 font-medium hover:underline flex items-center gap-0.5">
          ...
          {projectsLoading ? (
            ...
            <div className="py-8 text-center space-y-2">
              ...
              <p className="text-xs text-muted-foreground">No active projects created yet.</p>
              <Link href="/projects">
                  ...
                  >
                    <div className="space-y-1 min-w-0 flex-1 pr-3">
                      ...
                      <p className="text-[10px] text-muted-foreground truncate">
                        {project.client?.name ? `Client: ${project.client.name}` : `${taskCount} tasks`}
                      </p>
                    </div>

                    <div className="text-right shrink-0">
                      <span className="text-xs font-mono font-bold text-emerald-500">{completionPct}%</span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="toota-card p-5 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="w-6 h-6 rounded-full bg-blue-500/10 text-blue-500 flex items-center justify-center font-bold text-xs">✓</span>
          </div>
          <div className="flex items-baseline justify-between">
            <span className="text-3xl font-extrabold text-foreground">{totalCompletedTasks}</span>
          ...
          <div className="h-1.5 w-full bg-secondary rounded-full overflow-hidden">
            <div
              className="h-full bg-emerald-500 rounded-full transition-all"
              style={{ width: `${taskStats?.created ? Math.min(Math.round((totalCompletedTasks / taskStats.created) * 100), 100) : 0}%` }}
            />
          </div>
        </div>
        ...
        <div className="toota-card p-5 space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-muted-foreground">Time Tracked Week</span>
            <span className="w-6 h-6 rounded-full bg-purple-500/10 text-purple-500 flex items-center justify-center font-bold text-xs">⚡</span>
          </div>
          <div className="flex items-baseline justify-between">
            <span className="text-3xl font-extrabold text-foreground">{hoursWorked}h</span>
            <span className="bg-emerald-500/15 text-emerald-500 text-xs font-bold px-2.5 py-1 rounded-full">
              Active
            </span>
          </div>
          <p className="text-[10px] text-muted-foreground font-mono">Target: 40h / week</p>
          <div className="h-1.5 w-full bg-secondary rounded-full overflow-hidden">
            <div
        ...
        <div className="toota-card p-5 space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-muted-foreground">Revenue Overview</span>
            <span className="w-6 h-6 rounded-full bg-emerald-500/10 text-emerald-500 flex items-center justify-center font-bold text-xs">R</span>
          </div>
          <div className="flex items-baseline justify-between">
        ...
        <div className="toota-card p-5 space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-muted-foreground">Goal Streak</span>
            <span className="w-6 h-6 rounded-full bg-amber-500/10 text-amber-500 flex items-center justify-center font-bold text-xs">🔥</span>
          </div>
          <div className="flex items-baseline justify-between">
            <span className="text-3xl font-extrabold text-foreground">{currentStreak} Days</span>
          ...
          <div className="h-1.5 w-full bg-secondary rounded-full overflow-hidden">
            <div
              ...
              style={{ width: `${streakData?.longestStreak ? Math.min(Math.round((currentStreak / streakData.longestStreak) * 100), 100) : currentStreak > 0 ? 100 : 0}%` }}
            />
          </div>
        </div>
      </div>
      ...
      <div className="space-y-6 pt-4">
        <div className="flex items-center justify-between">
          ...
          <Link href="/tracklog" className="text-xs font-semibold text-muted-foreground hover:text-foreground flex items-center gap-1">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="toota-card space-y-4">
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      count > 0 && ratio >= 0.33 && ratio < 0.66 && "bg-emerald-400/70",
            ...
            <div className="flex items-center justify-between pt-2 border-t border-secondary/50">
              <div>
                <p className="text-xs font-bold text-foreground">Target: {tracklog?.target_hours ?? 8} hrs</p>
                <p className="text-[10px] text-muted-foreground">Range: 08:00 AM - 06:15 PM</p>
              </div>
              <button
          ...
          <div className="toota-card space-y-4">
            ...
            <div className="space-y-3 pt-1">
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  <div className="flex justify-between text-xs">
                    <span className="font-semibold text-foreground">Productive</span>
                    <span className="font-mono font-bold">{tracklog?.productive_hours ?? "—"}</span>
                  </div>
                  <div className="h-1.5 w-full bg-secondary rounded-full mt-1 overflow-hidden">
                    <div className="h-full bg-blue-500 rounded-full transition-all" style={{ width: `${productivePct}%` }} />
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="font-mono font-bold">{tracklog?.focused_hours ?? "—"}</span>
                  </div>
                  <div className="h-1.5 w-full bg-secondary rounded-full mt-1 overflow-hidden">
                    <div className="h-full bg-emerald-500 rounded-full transition-all" style={{ width: `${focusedPct}%` }} />
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  <div className="flex justify-between text-xs">
                    <span className="font-semibold text-foreground">Unproductive</span>
                    <span className="font-mono font-bold">{tracklog?.unproductive_time ?? "—"}</span>
                  </div>
                  <div className="h-1.5 w-full bg-secondary rounded-full mt-1 overflow-hidden">
                    <div className="h-full bg-amber-400 rounded-full transition-all" style={{ width: `${unproductivePct}%` }} />
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="toota-card space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs font-mono text-muted-foreground">Go Engine</span>
            </div>
            <div className="space-y-3">
              {tracklogLoading ? (
                <div className="space-y-2">
                  {[1, 2, 3].map((i) => <Skeleton key={i} className="h-6 w-full rounded-xl" />)}
                </div>
              ) : !tracklog?.apps || tracklog.apps.length === 0 ? (
                <p className="text-xs text-muted-foreground py-4 text-center">No app activity recorded yet.</p>
              ) : (
                tracklog.apps.map((app, idx) => (
                  <div key={idx} className="space-y-1">
                    <div className="flex justify-between text-xs font-medium">
                      <span className="text-foreground truncate max-w-[150px]">{app.name}</span>
                      <span className="font-mono text-muted-foreground">{app.duration}</span>
                    </div>
                    <div className="h-2 w-full bg-secondary rounded-full overflow-hidden">
                      <div className={cn("h-full rounded-full transition-all duration-300", app.color)} style={{ width: `${app.percent}%` }} />
                    </div>
                  </div>
                ))
              )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-2 toota-card space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-bold text-foreground">Active Window Feed</h3>
              <span className="text-xs font-mono text-muted-foreground">Electron PowerMonitor</span>
            </div>
            <div className="space-y-2 max-h-[320px] overflow-y-auto custom-scrollbar pr-1">
              {tracklogLoading ? (
                ...
                <p className="text-xs text-muted-foreground py-4 text-center">No window events recorded.</p>
              ) : (
                tracklog.events.map((evt, idx) => (
                  <div key={idx} className="flex items-center justify-between p-2.5 rounded-2xl bg-secondary/40 hover:bg-secondary transition-colors">
                    <div className="flex items-center gap-3 min-w-0">
                      <span className="text-xs font-mono text-muted-foreground shrink-0">{evt.time}</span>
                      <span className="text-xs font-bold text-foreground shrink-0 w-24 truncate">{evt.app}</span>
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <span className="bg-emerald-500/15 text-emerald-500 text-[10px] font-bold px-2 py-0.5 rounded-full">{evt.status}</span>
                    </div>
                  </div>
                ))
              )}
