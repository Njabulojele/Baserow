...
function WeekView({ date, events }: { date: Date; events: CalendarEvent[] }) {
  ...
  return (
    <div className="flex flex-col h-full bg-background">
      {/* Header */}
      <div className="grid grid-cols-8 border-b sticky top-0 z-30 bg-background">
        <div className="w-16 border-r bg-muted/5"></div>
        {days.map((day) => (
          <div
            key={day.toISOString()}
            className={cn(
          ...
          >
            ...
            <div
              className={cn(
                "text-xl font-semibold w-8 h-8 flex items-center justify-center rounded-full mx-auto mt-1",
                isSameDay(day, new Date()) &&
                  "bg-primary text-primary-foreground",
              )}
            >
      ...
      <ScrollArea className="flex-1">
        <div className="grid grid-cols-8 relative min-w-[800px]">
          {/* Time Labels */}
          <div className="w-16 border-r bg-background sticky left-0 z-20">
            {hours.map((hour) => (
              <div
                key={hour.toISOString()}
                className="h-20 text-xs text-muted-foreground text-right pr-2 pt-1 border-b relative"
              >
